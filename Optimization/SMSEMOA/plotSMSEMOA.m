function varargout = plotSMSEMOA(varargin)
%PLOTSMSEMOA M-file for plotSMSEMOA.fig
%      PLOTSMSEMOA, by itself, creates a new PLOTSMSEMOA or raises the existing
%      singleton*.
%
%      H = PLOTSMSEMOA returns the handle to a new PLOTSMSEMOA or the handle to
%      the existing singleton*.
%
%      PLOTSMSEMOA('Property','Value',...) creates a new PLOTSMSEMOA using the
%      given property value pairs. Unrecognized propertie are passed via
%      varargin to plotSMSEMOA_OpeningFcn.  This calling syntax produces a
%      warning when there is an existing singleton*.
%
%      PLOTSMSEMOA('CALLBACK') and PLOTSMSEMOA('CALLBACK',hObject,...) call the
%      local function named CALLBACK in PLOTSMSEMOA.M with the given input
%      arguments.
%
...
% Edit the above text to modify the response to help plotSMSEMOA

% Last Modified by GUIDE v2.5 08-Apr-2017 10:19:30

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @plotSMSEMOA_OpeningFcn, ...
                   'gui_OutputFcn',  @plotSMSEMOA_OutputFcn, ...
                   'gui_LayoutFcn',  [], ...
                   'gui_Callback',   []);
...
% --- Executes just before plotSMSEMOA is made visible.
function plotSMSEMOA_OpeningFcn(hObject, eventdata, handles, varargin)
...
% Choose default command line output for plotSMSEMOA
handles.output = hObject;
movegui(hObject,'center')                                               %Theisen  14.04.2016
% Update handles structure
guidata(hObject, handles);

% UIWAIT makes plotSMSEMOA wait for user response (see UIRESUME)
...
% b) Case : plotSMSEMOA(result) or plotSMSEMOA('pops.txt')
...
% c) Case : plotSMSEMOA(result, curGen)
...
function varargout = plotSMSEMOA_OutputFcn(hObject, eventdata, handles)
...
% --- Executes on button press in btn_End.
function btn_End_Callback(hObject, eventdata, handles)
% hObject    handle to btn_End (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
