...
% ------------
    ...
    em.M_max                             = x(1);
    Temp1                                = 2000:100:6000; % Discretization of the rotational speed in 100rpm increments from 1000 to 5000rpm
    em.n_eck                             = Temp1(x(2));
    em.P_max                             = ((em.n_eck * em.M_max *2* pi)/60)/1000; %[kW] Maximum power
    em.Type                              = x(11); % Electrical machine type 1: PMSM; 2: ASM
    em.n_max                             = 10000; % Maximum rotational speed
    em.noEM                              = 2; %default value for number of electric machines
    
    % Battery parameters
    %Bat.Voltage                        = x(3); % Not used in this case
    Bat.Useable_range                = x(3);
    Bat.Useable_capacity              = x(4);
    Bat.Type                             = x(10); % Battery type 1: Cylindrical; 2: Pouch
    Bat.SOC_start                        = 1;%0.5;     %[-] Initial battery SOC
    Bat.Voltage                         = 800;   %[V] Battery voltage
    Bat.Efficiency_power_electronics = 0.95;  %[-] Efficiency of the power electronics converter
    Bat.Charge_cycles =                 3000; % Quelle nachtragen benni M. Olaf Teichert
