function [ FuelCell ] = FuelCell_design( P_nom, V_nom )
...
% Stromdichte Zelle
i_nom_zelle = 0.175;%0.215;     % [A/cm2] Quelle: TUM Praktikum (Dummy) !Für 25 °C! --> Tendenziell höhere Dichte zu erwarten
...
I_max = A * i_zelle_end;%min([A * i_zelle_end, 450]);                    % Maximaler Strom [A], Quelle: Matlab 50 kW Stack, selbe max. Werte, weil keine Stacks parallel genommen. i_zelle_end müsste angepasst werden, weil Modell nicht mehr verwendet werden kann
...
I_end = A * 0.25;%min([A * 0.25, 450]);                           % Abschätzung, aus alten Daten Praktikum mit 0.25 A/cm^2 (Dummy) (Grafik, siehe Arbeit, Abb. )
...
% Volumenstrom Luft
V_lpm_air = (60000*R*T_nom*N*I_nom)/(2*z*F*101325*0.5*0.21); % Quelle Matlab Doku
...
FuelCell.Hysterese2         =    [0.4 0.35];
FuelCell.Hysterese3         =    [0.45 0.4];
FuelCell.Hysterese4         =    [0.5 0.45];
FuelCell.Hysterese5         =    [0.55 0.5];
end
