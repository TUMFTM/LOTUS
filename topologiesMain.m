...
ifOptimized = false; % ifOptimized simulates using an optimized vehicle, the fuel type and drivetrain can be chosen in a GUI
% else, you need to chose a fuel type in FuelOptimisation

ratioSelect = 0.8:0.1:1.2; %ratios for sensitivity analysis
i3 = 3; % comment out to perform sensitivity analysis
for i1 = 1:5 % iterate all topologies
    for i2 = 1:4 %iterate driving cycles
        %         for i3 = 1:length(ratioSelect) % uncomment for sensitivity
        
        [Fuel_type, vehicle_param, ifElectric, Vehicle] = FuelOptimisation(ifOptimized, 7); % FuelOptimisation creats an all the parameters needed to start Prametrizing
        ...
        switch i1
            case 1 % DAF CF Electric (Direct Drive)
                temp = load('engineMap_DAF.mat');
                Param.final_drive.ratio = 17.5*ratioSelect(i3);
                Param.transmission = Transmission_gearing(1, 1, false, false, Param.final_drive.ratio);
            case 2 % E-Force One (Three Speed)
                temp = load('engineMap_EForce.mat');
                Param.final_drive.ratio = 5*ratioSelect(i3);
                Param.transmission = Transmission_gearing(1.7284, 3, false, false, Param.final_drive.ratio);
            case 3 % Nikola Tre (Single Speed)
                temp = load('engineMap_Nikola.mat');
                Param.final_drive.ratio = 19.44*ratioSelect(i3);
                Param.transmission = Transmission_gearing(1, 1, false, false, Param.final_drive.ratio);
            case 4 % Tesla Semi (Direct Drive)
                temp = load('engineMap_Tesla.mat');
                Param.final_drive.ratio = 11.75*ratioSelect(i3);
                Param.transmission = Transmission_gearing(1, 1, false, false, Param.final_drive.ratio);
            case 5 % ZF AxTrax (Direct Drive)
                temp = load('engineMap_ZF.mat');
                Param.final_drive.ratio = 22.5*ratioSelect(i3);
                Param.transmission = Transmission_gearing(1, 1, false, false, Param.final_drive.ratio);
        end
        ...
        [Param] = Weights_calculation(Param, 'BEV');
        ...
        cycleSelect = [2, 12, 20, 21];
        Param.dcycle = cycleSelect(i2);
        ...
        consumption(i1,i2) = Param.VSim.Consumption_kWh;
        efficiencyDC(i1,i2) = mean(Results.Elektro_LKW.signals(5).values(Results.Elektro_LKW.signals(5).values>1e-3), 'omitnan');
        efficiency(i1,i3) = mean(Results.Elektro_LKW.signals(5).values(Results.Elektro_LKW.signals(5).values>1e-3), 'omitnan');
        [t_60_80] = Acceleration(Param.VSim);
        elasticity(i1, i3) = t_60_80;
        
        OperatingPointsEngineMap
        %     end % uncomment for sensitivity
    end
end
