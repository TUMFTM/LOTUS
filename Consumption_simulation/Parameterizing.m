...
% ------------
%% Sources
% [1]	M. Fries, M. Sinning, M. Lienkamp, und M. Höpfner, Virtual Truck - A Method for Customer Oriented Commercial Vehicle Simulation, 2016.
% [2]	C. Mährle et al, “Bayerische Kooperation für Transporteffizienz - Truck2030 - Status Report 2016 -,” 2017.
...
% [7]	M. Wietschel und et. al, “Machbarkeitsstudie zur Ermittlung der Potentiale des Hybrid-Oberleitungs-Lkw,” Fraunhofer Institut für System und, Karlsruhe, 2017.
% 
% ------------
...
%     vehicle.payload = 14000; % [kg] medium weight for autmotive goods or food
    ...
    tires.roll_drag_coeff.total = 0.0043;         % [-] Optimum concept rolling resistance coefficient (assumption)
    ...
    if ifOptimized % When the parameters are optimized
        ...
        if ~ifElectric % All drivetrains except electric
            ...
            [SOC_addition,critical_altitude_difference,distance_PPC_altitude_difference,...
             ...
             v_max_electrical_drive_only, T_distance_LPS_up, T_distance_LPS_down] = Hybrid_operating_strategy(x, Fueltype);
            ...
            elseif Fueltype == 13
                [ FuelCell ] = FuelCell_design(150, 400); %  FuelCell_design(max power, nominal voltage), function of fuel cell design
            end
        end
...
%         final_drive.ratio = 2.846;                    %[-] Differential gear ratio. Also available: 2.71  / 2.846 / 2.92 / 3.077                                                         
       final_drive.ratio = 10;                      % Geschätzt Tesla Semi
...
%         transmission = Transmission_gearing(1, 1, false, false, final_drive.ratio); % Tesla Semi (assumption)
        transmission = Transmission_gearing(1, 1, false, false, final_drive.ratio); % Nikola Two  (assumption)
...
%         transmission = Transmission_gearing(5, 3, 0, 0, final_drive.ratio);
        ...
        M_el                             = 1000;   % Maximum torque up to which the vehicle is driven purey electrically 
        SOC_el                           = 0.8;
       ...
       % Battery design, for hybrid and non-hybrid vehicles  [5]
        ...
        Bat.Useable_range                = 0.8;   % Battery DoD
...
%             em.P_max = 192*4;                                    % [kw] Maximum power of Tesla Semi
        ...
        % Battery design, for pure electric vehicles
            ...
            Bat.Useable_capacity = 1000;                       % [Ah] Opti VR
...
%         WPT.SOC_electric_only = 0.3;  % Minimal SOC for pure electric driving
        ...
        % Fuell cell design
        if Fueltype == 13
            FuelCell = FuelCell_design(100, 800); %  FuelCell_design(P_max, Nominal voltage)
...
%% Simulation selection
    switch Fueltype
        ...
        case {8, 9, 10, 11} % Dual-Fuel (hybrid)
        ...
        case 12 % Elektro mit Wireless Power Transfer
            name = 'HDVSim_BEV_WPT_Truck';

        case 13 % Elektro mit Wireless Power Transfer
...
%% Classes initialization
    ...
    % Wird für Standalone verwendet, sonst in NFZEP berechnet
